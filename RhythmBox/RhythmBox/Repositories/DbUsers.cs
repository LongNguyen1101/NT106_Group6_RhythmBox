...
namespace RhythmBox.Repositories
{
	public class DbUsers : IDbUsers
	{
		...
		public async Task newUserUploadAsync(RhythmboxdbContext context, string userName, string email, string password, string birthday, string gender)
		{
		...
		public async Task userDownloadAsync(RhythmboxdbContext context, string userName, string email)
		{
			var users = context.Users
								.Where(con => (con.Email == email));
